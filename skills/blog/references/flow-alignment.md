...
3. **URL with retrieval date.** Full URL plus the italicised qualifier `*— retrieved D Month YYYY*` (e.g. `*— retrieved 9 June 2026*`) in a bibliography or source block at the end of the post. The em-dash separates the cited title from the retrieval qualifier; the date is the human form, not ISO. This is provenance discipline; it lets future readers (and AI crawlers) verify the source still says what was claimed. **Exception — first-party / own research:** the brand's own published datasets are cited by name + edition vintage only ("June 2026"); no retrieved date (the publication date IS the provenance) and never `(n=…)` sample-size notation anywhere (citation, chart source line, caption).
