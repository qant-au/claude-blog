...
| `--from-queue` (rewrite only, v1.9.2) | unset | Queue mode for `/blog rewrite`. Reads every draft in `qant-blog-drafts.brands/{brand_slug}/drafts/` flagged with `review_state == "needs_rewrite"` and rewrites each in sequence, clearing the flag on success. Requires `--brand <slug>`. Skips the positional `<file-path>` argument. See `skills/blog-rewrite/SKILL.md` Phase 0.3 for the full workflow. The flag is set by the Blog Manager UI's "Flag for rewrite" action (qnt-045) — until that ships, set the field by hand for testing per the pattern in the rewrite skill. |
